...
;jmp st1 - takes 3 bytes followed by nop that is 4 bytes
         nop
;IVT entry for Remote Control - address for entry is 20H x 4 is 00080H
    dw isr_2min;  Instruction Pointer Value = 2 bytes 
    dw 0000      ;  CS is 2 bytes          
    
;IVT entry for 15 Seconds - address for entry is 21H x 4 is 00084H
    dw isr_eoc;  Instruction Pointer Value = 2 bytes
    dw 0000      ;  CS is 2 bytes
...
;select channel
x0:		  mov       di,1
;di is made 1 to check whether isr_eoc is executed
          mov		al,dh
		  out		04h,al
;give ale
          or        al,00100000b
